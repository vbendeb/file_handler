...
```
By default `${ROOT}` is set to `/backup_disk/home/${USER}/docs/pictures`, the
command line option `-r` allows to change this.
...
on it. If more than one removable device is found the script reports the error
and terminates.
...
```
usage: file_handler [-h] [-a] [-t TIME_OFFSET] [-r ROOT]
...
optional arguments:
  ...
  -r ROOT, --root ROOT  root directory of the photo library. By default set to /backup_disk/home/${USER}/docs/pictures
```
